...
import { getUserToken, getCorpId } from "@/utils/urlSearch"
    ...
    return new Promise((resolve) => {
        ...
        if (token) {
            getInfo().then((info) => {
                ...
                messageError(error.msg || error.message || "获取用户数据失败")
                loginDing();
            })
        } else {
            loginDing();
        ...
        function loginDing() {
            ...
            if (ding && getCorpId()) {
                actions.dingLogin().then((info) => {
                    ...
                    messageError(error.msg || error.message || "钉钉免登录失败")
                    loginToken();
                })
            } else {
        ...
        function loginToken() {
            if (userToken) {
                actions.tokenLogin(userToken).then((info) => {
                    ...
                    messageError(error.msg || error.message || "用户登录失败")
                    resolve(init.NOLOGIN);
                })
            } else {
                window.titlePrompt = "请通过扫描二维码进入\r\n";
