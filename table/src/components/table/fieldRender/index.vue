<template>
    ...
    <div v-if="field.render == 'tag' && fieldValue !== ''">
        <el-tag :type="getTagType(fieldValue, field.custom)" :effect="Nullish(field.effect, 'light')" :size="Nullish(field.size,'default')">{{
        ...
        <template v-if="Array.isArray(fieldValue)">
            <template v-for="(tag, idx) in fieldValue" :key="idx">
                <el-tag
                    ...
                    :effect="Nullish(field.effect ,'light')"
                    :size="Nullish(field.size , 'default')"
                >
                    {{ field.replaceValue ? Nullish(field.replaceValue[tag] , tag) : tag }}</el-tag
                >
            </template>
        </template>
        <template v-else>
            <el-tag
                ...
                :size="Nullish(field.size , 'default')"
            >
                {{ field.replaceValue ? Nullish(field.replaceValue[fieldValue],fieldValue) : fieldValue }}</el-tag
            >
        </template>
    </div>
    ...
    <div v-if="field.render == 'datetime'">
        {{ !fieldValue ? '-' : timeFormat(fieldValue, Nullish(field.timeFormat , undefined)) }}
    </div>
    ...
    <div v-if="field.render == 'button'">
        <el-button @click="field.click(row)">查看</el-button>
    </div>
...
import { fullUrl, arrayFullUrl,Nullish } from '@/utils/common'
...
if (props.property.indexOf('.') > -1) {
    ...
    for (let index = 1; index < fieldNameArr.length; index++) {
        val.value = val.value ? Nullish(val.value[fieldNameArr[index]] , '') : ''
    }
