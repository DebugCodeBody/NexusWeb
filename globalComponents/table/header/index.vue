<template>
    ...
    <div v-bind="$attrs" class="table-header">
        <el-tooltip v-if="props.buttons.includes('refresh')" :content="baTable.I18n('refresh')" placement="top">
            <el-button v-blur @click="onAction('refresh', { loading: true })" color="#40485b"
                class="table-header-operate" type="info">
                <Icon name="fa fa-refresh" />
            </el-button>
        </el-tooltip>
        <el-tooltip v-if="props.buttons.includes('add') && auth('add')" :content="baTable.I18n('add')" placement="top">
        ...
        <el-tooltip v-if="props.buttons.includes('edit') && auth('edit')" :content="baTable.I18n('Edit selected row')"
            placement="top">
        ...
        <el-popconfirm v-if="props.buttons.includes('delete') && auth('del')" @confirm="onAction('delete')"
            :confirm-button-text="baTable.I18n('delete')" :cancel-button-text="baTable.I18n('Cancel')" confirmButtonType="danger"
            :title="baTable.I18n('Are you sure to delete the selected record?')" :disabled="!enableBatchOpt">
            <template #reference>
                <div class="mlr-12">
                    <el-tooltip :content="baTable.I18n('Delete selected row')" placement="top">
        ...
        <el-tooltip v-if="props.buttons.includes('unfold')"
            :content="(baTable.table.expandAll ? baTable.I18n('shrink') : baTable.I18n('open')) + baTable.I18n('All submenus')" placement="top">
            <el-button v-blur @click="changeUnfold" class="table-header-operate"
                ...
                <span class="table-header-operate-text">{{
                    baTable.table.expandAll ? baTable.I18n('Shrink all') : baTable.I18n('Expand all')
                }}</span>
            </el-button>
        </el-tooltip>
            ...
            <div class="table-search-button-group"
                ...
                <el-dropdown v-if="props.buttons.includes('columnDisplay')" :max-height="380" :hide-on-click="false">
                    <el-button class="table-search-button-item"
                        :class="props.buttons.includes('comSearch') ? 'right-border' : ''" color="#dcdfe6" plain>
                ...
                <el-tooltip v-if="props.buttons.includes('comSearch')" :disabled="baTable.table.showComSearch"
                    :content="baTable.I18n('Expand generic search')" placement="top">
                    <el-button class="table-search-button-item"
