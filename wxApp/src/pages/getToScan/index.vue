    ...
    methods: {
        ...
        onClickScan() {
            ...
            scanCode().then((result) => {
                ...
                showLoading("正在提交");
                sboOrderScan(result).then((response: any) => {

                    response.success = response.msg.indexOf("成功") > -1;

                    const index = this.itemList.findIndex((elem) => elem.orderid == response.orderid);
                    if (index == -1) {
                    ...
                    showErrorToast(err.message || "扫描失败");
                })
