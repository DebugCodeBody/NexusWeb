 ...
 * 默认按钮组
 */
export const defaultOptButtons = (optButType: DefaultOptButType[] = ['weigh-sort', 'edit', 'delete'], options?: any): OptButton[] => {
    ...
    for (const key in optButType) {
        if (optButtonsPre.has(optButType[key])) {
            ...
            const value = optButtonsPre.get(optButType[key])!;

           
            value.npTooltip = !(options && options.npTooltip);

            optButtons.push(value)
        }
    }
    return optButtons
}
