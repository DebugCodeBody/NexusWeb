<template>
    <pop-dialog v-model="person.show" :title="title[person.action]" style="width: 30%;" @confirm="confirm" @cancel="cancel" ref="Dialog" >
...
import { personAdd, personEdit } from "@/api/person"
...
async function confirm() {
    ...
    try {
        ...
        if (person.action == "add") {
            await personAdd(form)
        } else {
            await personEdit(person.item.id, form);
        }
        Dialog.close();
    } catch (err: any) {
        Dialog.stopSubmit();
